...
sim.es.mean <- seq(0,1,.2)    # mean effect size, Cohen's d 6 elements
#sim.es.sd <- seq(0,.6,.2) # heterogeneity of es (tau) 
sim.n.obs <- c(10, 20, 40, 60, 80, 100)   # sample size per cell 5 elements
sim.r.var <- seq(0,1,.2)      # correlation r between multiple vars 6 elements
...
for (i in 1:length(sim.es.mean) ) {
	for (j in 1:length(sim.r.var)) {
		for (k in 1:length(sim.n.obs) ) {
			sim.i = sim.i + 1
			sims = rbind(sims,c(sim.i,sim.es.mean[i],sim.r.var[j],sim.n.obs[k]) )
}}}
sims = data.frame(sims)
colnames(sims) = c("run","es.mean","r.var","n.obs")
dim(sims)
# sims IS A DATA FRAME CONTAINING ALL COMBINATIONS OF STUDY CONDITIONS
...
# Vector of sample1 is 1:15, sample2 is 16:30, etc
...
for (i.patch in 1:7) {
      ...
      length(sub_exp)
      length(sub_control)
      tval <- t.test(sub_exp, sub_control, paired=FALSE,var.equal=TRUE)$statistic
